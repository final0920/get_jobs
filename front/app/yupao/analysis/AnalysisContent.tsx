...
const CATEGORY_COLORS = [
  "#3b82f6", "#10b981", "#f59e0b", "#ef4444", "#6366f1", "#22c55e",
  "#fb7185", "#a78bfa", "#f97316", "#06b6d4", "#4ade80", "#2dd4bf", "#f472b6", "#64748b",
]

function ChartCanvas({
  type, labels, data, title, color = "#3b82f6", colors,
}: {
  type: "pie" | "bar" | "line"
  labels: string[]
  data: number[]
      ...
      try {
        ...
        chartRef.current = new Chart(ctx, {
          ...
          options: {
            ...
            plugins: { legend: { display: type === "pie" }, title: { display: !!title, text: title } },
...
export default function AnalysisContent({ showHeader = false }: { showHeader?: boolean }) {
  ...
  const loadList = async (toPage = page, toSize = size) => {
    try {
      ...
      const res = await fetch(`${API_BASE}/api/yupao/list?${params.toString()}`)
      ...
      console.error("fetch yupao list failed", e)
    }
  }
  ...
  const loadStats = async () => {
    try {
      ...
      if (maxK) params.set("maxK", String(Number(maxK)))
      if (keyword) params.set("keyword", keyword)
      const res = await fetch(`${API_BASE}/api/yupao/stats?${params.toString()}`)
      const data: StatsResponse = await res.json()
      setStats(data)
    } catch (e) {
      console.error("fetch yupao stats failed", e)
    } finally {
  ...
  const exportCSV = async () => {
    try {
      ...
      const rows = all.map((it) => [
        it.companyName || "", it.jobTitle || "", it.salary || "", it.location || "",
        it.experience || "", it.degree || "", it.deliveryStatus || "", it.jobLink || "", formatDateOnly(it.createTime),
      ])
      ...
      a.href = url
      a.download = `yupao_jobs_${Date.now()}.csv`
  ...
  const kpiCards = useMemo(() => {
    ...
    const avgFromItems = (() => {
      ...
      return Math.round((ks.reduce((a, b) => a + b, 0) / ks.length) * 10) / 10
    })()
    return [
      { title: "总岗位数", value: k?.total ?? 0 },
      { title: "已投递", value: k?.delivered ?? 0 },
      { title: "未投递", value: k?.pending ?? 0 },
      { title: "平均月薪(K)", value: (k?.avgMonthlyK ?? avgFromItems ?? 0) },
    ]
  }, [stats, items])
  ...
  return (
    <div className="space-y-8">
      {showHeader && (
        <PageHeader title="鱼泡 投递分析" subtitle="基于 yupao_data 表的统计图与列表分析" icon={<BiBarChart size={28} />} />
      )}
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
            <div className="space-y-2"><Label>地区</Label><Input placeholder="如：武汉" value={location} onChange={(e) => setLocation(e.target.value)} /></div>
            <div className="space-y-2"><Label>经验</Label><Input placeholder="如：3-5年" value={experience} onChange={(e) => setExperience(e.target.value)} /></div>
            <div className="space-y-2"><Label>学历</Label><Input placeholder="如：本科" value={degree} onChange={(e) => setDegree(e.target.value)} /></div>
            <div className="space-y-2"><Label>最低K</Label><Input placeholder="如：10" value={minK} onChange={(e) => setMinK(e.target.value)} /></div>
            <div className="space-y-2"><Label>最高K</Label><Input placeholder="如：30" value={maxK} onChange={(e) => setMaxK(e.target.value)} /></div>
            <div className="space-y-2"><Label>关键词</Label><Input placeholder="公司或岗位关键词" value={keyword} onChange={(e) => setKeyword(e.target.value)} /></div>
          </div>
          <div className="mt-4 flex items-center gap-3">
            <Button variant="default" onClick={() => loadStats()} disabled={loadingStats}><BiRefresh className="mr-1" /> 刷新统计</Button>
            <Button variant="outline" onClick={() => loadList(1, size)}><BiBriefcase className="mr-1" /> 刷新列表</Button>
            <Button variant="outline" onClick={exportCSV} disabled={exporting}><BiDownload className="mr-1" /> 导出CSV</Button>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          <CardHeader><CardTitle className="text-base flex items-center gap-2"><BiBarChart /> 投递状态分布</CardTitle><CardDescription>按 delivery_status 聚合</CardDescription></CardHeader>
          <CardContent>{stats ? (<ChartCanvas type="pie" labels={stats.charts.byStatus.map((x) => x.name)} data={stats.charts.byStatus.map((x) => x.value)} colors={CATEGORY_COLORS} />) : (<div className="text-muted-foreground">加载中...</div>)}</CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="text-base flex items-center gap-2"><BiBarChart /> 城市TOP10</CardTitle><CardDescription>按地区聚合</CardDescription></CardHeader>
          <CardContent>{stats ? (<ChartCanvas type="bar" labels={stats.charts.byCity.map((x) => x.name)} data={stats.charts.byCity.map((x) => x.value)} color="#3b82f6" />) : (<div className="text-muted-foreground">加载中...</div>)}</CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="text-base flex items-center gap-2"><BiBarChart /> 公司岗位数TOP10</CardTitle><CardDescription>按公司名称聚合</CardDescription></CardHeader>
          <CardContent>{stats ? (<ChartCanvas type="bar" labels={stats.charts.byCompany.map((x) => x.name)} data={stats.charts.byCompany.map((x) => x.value)} color="#10b981" />) : (<div className="text-muted-foreground">加载中...</div>)}</CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="text-base flex items-center gap-2"><BiBarChart /> 经验分布</CardTitle><CardDescription>不同经验要求的岗位数</CardDescription></CardHeader>
          <CardContent>{stats ? (<ChartCanvas type="bar" labels={stats.charts.byExperience.map((x) => x.name)} data={stats.charts.byExperience.map((x) => x.value)} color="#f59e0b" />) : (<div className="text-muted-foreground">加载中...</div>)}</CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="text-base flex items-center gap-2"><BiBarChart /> 学历分布</CardTitle><CardDescription>不同学历要求的岗位数</CardDescription></CardHeader>
          <CardContent>{stats ? (<ChartCanvas type="bar" labels={stats.charts.byDegree.map((x) => x.name)} data={stats.charts.byDegree.map((x) => x.value)} color="#6366f1" />) : (<div className="text-muted-foreground">加载中...</div>)}</CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="text-base flex items-center gap-2"><BiLineChart /> 薪资区间分布</CardTitle><CardDescription>基于中位数K的桶聚合</CardDescription></CardHeader>
          <CardContent>{stats ? (<ChartCanvas type="line" labels={(computedSalaryBuckets.length ? computedSalaryBuckets : stats.charts.salaryBuckets).map((x) => (x as any).bucket)} data={(computedSalaryBuckets.length ? computedSalaryBuckets : stats.charts.salaryBuckets).map((x) => x.value)} color="#ef4444" />) : (<div className="text-muted-foreground">加载中...</div>)}</CardContent>
        </Card>
      </div>

      <Card>
        <CardHeader><CardTitle className="text-base">岗位列表</CardTitle><CardDescription>分页展示符合筛选条件的岗位</CardDescription></CardHeader>
        <CardContent>
              ...
              <tbody>
                {items.map((it, idx) => (
                  <tr key={`${it.jobId}-${idx}`} className="border-t">
                    ...
                    <td className="py-2 px-3 whitespace-nowrap"><span className={badgeClass("delivery", it.deliveryStatus)}>{it.deliveryStatus || ""}</span></td>
                    <td className="py-2 px-3 whitespace-nowrap">{it.jobLink ? (<a href={it.jobLink} target="_blank" rel="noreferrer" className="text-primary hover:underline">打开</a>) : (<span className="text-muted-foreground">-</span>)}</td>
          ...
          <div className="mt-4 flex items-center gap-2">
            ...
            <Input className="w-20" value={inputPage} onChange={(e) => setInputPage(e.target.value)}
              onBlur={() => { const p = Number(inputPage); const s = Number(size); if (!isNaN(p) && p > 0) loadList(p, s) }} />
            <Label className="text-sm">每页条数</Label>
            <Input className="w-24" value={inputSize} onChange={(e) => setInputSize(e.target.value)}
              onBlur={() => { const p = Number(page); const s = Number(inputSize); if (!isNaN(s) && s > 0) loadList(p, s) }} />
            <Button variant="outline" onClick={() => loadList(Number(page), Number(size))}>跳转</Button>
